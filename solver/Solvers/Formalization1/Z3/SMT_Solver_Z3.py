...
OUPTUP_BASE_PATH = "/".join(os.path.dirname(__file__).split("/")[:-3])
...
class Z3_Solver_Int_Parent(ManuverSolver):  # ManeuverProblem):
                ...
                self.solver.add(
                    (
                        sum(
                            [self.a[alphaCompId * self.nrVM + j]]
                            + [
                                self.a[_compId * self.nrVM + j]
                                for _compId in notInConflictCompsIdList
                            ]
                        )
                    )
                    == (
    ...
    def run(self):
        ...
        if status == sat:
            ...
            output += f"Price for each machine {vms_price} \n"
            minprice = 0
            for p in vms_price:
                minprice = minprice + p
            output += f"Min price {minprice}\n"
            output += "Type for each machine\n"
            vms_type = []
            for k in range(self.nrVM):
        ...
        try:
            os.remove(f"{OUPTUP_BASE_PATH}/Output/web_output.txt")
        except OSError:
            pass

        with open(f"{OUPTUP_BASE_PATH}/Output/web_output.txt", "w+") as f:
            f.write(output)
